...
// same idea as the pool class:
std::array<PositionComponent, 10> AoS;
...
int size = 1000000;
std::vector<MagicStatComponent> magicStats(size);
double deltaTime = 0.0012f;
int main(){
    for(int i = 0; i < size; i++){
        magicStats[i].MP = std::min(magicStats[i].MP + magicStats[i].wisdom * deltaTime,
                                    static_cast<double>(magicStats[i].maxMP));
    }    
}